...
def extract(path: Path, res: SourceResult, ctx: dict) -> None:
    ...
    if name and not text.is_null_marker(name):
        add("full_name", text.nfc(str(name)), name)
    if doc.get("bio"):
        add("headline", text.nfc(str(doc["bio"])), doc["bio"])

    login = doc.get("login")
    gh_url = doc.get("html_url") or (f"https://github.com/{login}" if login else None)
    if gh_url:
        add("links.github", urls.classify(str(gh_url))[1], gh_url)
    if doc.get("blog"):
        bucket, cleaned = urls.classify(str(doc["blog"]))
        add(f"links.{bucket}", cleaned, doc["blog"])

    if doc.get("email"):
        norm = emails.normalize(doc["email"])
        if norm:
            add("emails", norm, doc["email"])
    ...
    for lang in langs:
        ...
        add("skills", {"name": name_, "canonical": canonical}, lang,
            method="derived:github_languages_v1")
